...
useEffect(() => {
  ...
  const diagnosisId =
    typeof data.diagnosisId === "string"
      ? data.diagnosisId.trim()
      : "";

  /*
   * 通常の /upload アクセスで既に診断済みの場合は、
   * 最新の診断結果へ戻します。
   *
   * ?mode=retry が付いている場合だけ、
   * 再診断としてUpload画面を表示します。
   */
...
<div className="mt-3 overflow-hidden rounded-[16px] border border-black/[0.06] bg-[#FAFAFA]">
  ...
  <p className="mt-1 text-[9px] leading-4 text-black/80">
    画像は一般公開されず、AI処理のため外部AIサービスを利用します。
        ...
        <div className="fixed inset-x-0 bottom-0 z-50 mx-auto w-full max-w-[480px] border-t border-black/10 bg-white px-4 pb-[max(14px,env(safe-area-inset-bottom))] pt-3 shadow-[0_-4px_16px_rgba(15,23,42,0.04)]">
