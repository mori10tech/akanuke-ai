...
const SAFE_AI_RECOMMENDED_TARGET =
  "AIにおまかせ（写真から本人に似合う垢抜け方向）";
...
const SUSPICIOUS_INSTRUCTION_PATTERNS =
  [
    ...
    /\b(system|developer|assistant)\s*(prompt|message|instruction)s?\b/gi,
    /\bjailbreak\b/gi,
    /指示を無視/gi,
    /命令を無視/gi,
    /以前の指示/gi,
...
export function createAfterImagePrompt(
  sourceAnalysis: AkanukeAnalysis,
) {
  /*
   * After生成プロンプトへ展開する文字列は、
   * DB保存済みの診断結果であっても
   * そのまま信頼せず安全化します。
   *
   * ユーザー入力由来の文字列や、
   * AI診断結果に予期しない命令文が混入しても、
   * 画像生成AIへの追加指示として機能しにくくします。
   */
  const analysis: AkanukeAnalysis =
    {
      ...sourceAnalysis,
